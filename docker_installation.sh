...
macOS:
  - Installs Docker Desktop from a local Docker.dmg you provide (arg or DOCKER_DMG)
Windows:
  - Prints the official command-line install command for Docker Desktop (you still must download the .exe)

Examples:
  sudo ./install-docker.sh
  sudo ./install-docker.sh --mac-dmg /path/to/Docker.dmg
EOF
}
...
install_macos_desktop() {
  if [[ -z "${MAC_DMG}" || ! -f "${MAC_DMG}" ]]; then
    echo "macOS detected. Provide Docker.dmg via --mac-dmg or DOCKER_DMG."
    exit 1
  fi
  ...
  echo "Docker Desktop installed. Launch Docker.app once to finish setup."
}
  ...
  Linux)
    ...
    case "${ID}" in
      ...
      fedora)
        ...
        ;;
      *)
        echo "Linux distro not supported by this script: ID=${ID}"
        echo "See Docker docs for your distro: https://docs.docker.com/engine/install/"
        exit 2
        ;;
    esac
    ;;
